        ...
        <div class="bg-white p-8 rounded-lg shadow-md w-full">
            ...
            @if ($user->avatar)
                <div class="mt-2 flex justify-center">
                    <img src="{{ asset('storage/' . $user->avatar) }}" alt="{{ $user->name }} Avatar" class="w-32 h-32 object-cover rounded-full">
                </div>
            @endif
            ...
            @forelse ($jobs as $job)
                <div class="flex justify-between items-center border-b-2 border-gray-200 py-2">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <!-- Delete Form -->
                        <form method="POST" action="{{ route('jobs.destroy', $job->id) }}?from=dashboard" onsubmit="return confirm('Are you sure want to delete this job?')" >
                            @csrf
                            @method('DELETE')
                ...
                <div class="mt-4 bg-gray-100 p-2 ">
                    ...
                    <div class="py-2">
                        ...
                        <p class="text-gray-800"><a href="{{ asset('storage/' . $applicant->resume_path) }}" class="text-blue-500 text-sm hover:underline" download><i class="fas fa-download"></i> Download Resume </a></p>
                    {{-- Delete Applicant --}}
                    <form 
                        method="POST" 
                        action="{{ route('applicant.destroy', $applicant->id) }}"
                        onsubmit="return confirm('Are you sure want to delete this applicant?')"
                    >
                        @csrf
                        @method('DELETE')

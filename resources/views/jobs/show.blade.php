<x-layout>
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <section class="md:col-span-3">
                    <div class="rounded-lg shadow-md bg-white p-3">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex space-x-3 ml-4">
                                ...
                                <!-- Delete Form -->
                                <form method="POST" action="{{ route('jobs.destroy', $job->id) }}" onsubmit="return confirm('Are you sure want to delete this job?')" >
                                    @csrf
                                    @method('DELETE')
                            ...
                            <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center bg-gray-900/50">
                                <div @click.away="open = false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                                    ...
                                    <form method="POST" action="{{ route('applicant.store', $job->id)}}" enctype="multipart/form-data">
                                        ...
                                        <button @click="open = false" class="bg-gray-300 hover:bg-gray-400 text-black px-4 py-2 rounded-md">Cancle</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <form method="POST" action="{{ $isBookmarked ? route('bookmarks.destroy', $job->id) : route('bookmarks.store', $job->id) }}" class="mt-10">
                    @csrf
                    @if ($isBookmarked)
                        @method('DELETE')
                        <button class="bg-red-500 hover:bg-red-600 text-white font-bold w-full px-4 py-2 rounded-full flex items-center justify-center">
                            <i class="fas fa-bookmark mr-3"></i> Remove Bookmark   
                        </button>
                    @else
                        <button class="bg-blue-500 hover:bg-blue-600 text-white font-bold w-full px-4 py-2 rounded-3xl flex items-center justify-center">
                            <i class="fas fa-bookmark mr-3"></i> Bookmark Listing   
                        </button>
                    @endif
                </form>
                @else
                    <button class="bg-gray-400 text-black font-bold w-full px-4 py-2 rounded-3xl flex items-center justify-center mt-4">
                            <i class="fas fa-info-circle mr-3"></i>
